...
namespace switch_logo_patcher
{
    public partial class Form1 : Form
    {
        ...
        private void Disk_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                cheminDossier = Path.Combine(Path.GetPathRoot(lecteurSelectionne), "atmosphere", "exefs_patches");
        ...
        private void Create_button_Click(object sender, EventArgs e)
        {
            ...
            if (Disk.SelectedItem != null)
            {
                ...
                string resDirectoryPath = Path.Combine(selectedDrive, "bootloader", "res");
                ...
                if (Background_hekate_pictureBox.Image != null)
                {
                    if (Background_hekate_pictureBox.Image != null)
                    {
                        ...
                        string selectedDrivePath = Path.GetPathRoot(Disk.SelectedItem.ToString());
                        string bootloaderResPath = Path.Combine(selectedDrivePath, "bootloader", "res");
                        Directory.CreateDirectory(bootloaderResPath);
                        string backgroundImagePath = Path.Combine(bootloaderResPath, "background.bmp");
                    ...
                    {
                        MessageBox.Show("Aucune image à convertir.", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                if (SplashScreen_pictureBox.Image != null)
                {
                    string selectedDrivePath = Path.GetPathRoot(Disk.SelectedItem.ToString());
                    string savePath = Path.Combine(selectedDrivePath, "bootloader", "res");
                    Directory.CreateDirectory(savePath);
                    string bootLogoPath = Path.Combine(savePath, "bootlogo.bmp");
                ...
                if (Background_hbmenu_pictureBox.Image != null)
                {
                    ...
                    string selectedDriveLetter = Disk.SelectedItem.ToString().Substring(0, 1);
